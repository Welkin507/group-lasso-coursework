...
% 利用增广拉格朗日法求解 Group LASSO 问题的对偶问题
...
if ~isfield(opts, 'alpha'); opts.alpha = 0.0025; end
...
while k < opts.maxit && abs(f - fp) > opts.ftol && nrmC > opts.gtol
    ...
    % Inner loop, solve the subproblem for y and z.
    % y^{i+1} =& y^i - \alpha \left( y^i + b + \sigma A \left( A^T y^i + z^i - \frac{1}{\sigma} x^k \right) \right) \\
    % z^{i+1} =& \mathrm{Proj}_{\left\{ z:\left\| z \right\| _{\infty ,2}\le \mu \right\}}\left( z^i-\alpha \sigma \left( A^Ty^i+z^i-\frac{1}{\sigma}x^k \right) \right) \\
