...
% This function implements the FISTA method for the group LASSO problem.
...
if ~isfield(opts, 'maxit'); opts.maxit = 2000000; end
...
while k < opts.maxit && nrmG > opts.gtol && abs(f - fp) > opts.ftol
    ...
    % 特别地，除了每次迭代开始处的收敛条件外，如果连续 20 步的函数值最小值比 20 步之前的函数值超过阈值，
    % 则停止内层循环。
    stop_t = 200;
    if k > stop_t && min(out.fvec(k-stop_t+1:k)) - out.fvec(k-stop_t) > opts.ftol
        break;
    end
end
